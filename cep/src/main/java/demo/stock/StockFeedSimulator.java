...
public class StockFeedSimulator {
	...
	public static void main(String[] args) {
		...
		final KieSession session = container.newKieSession("ksession-rules");
		KieRuntimeLogger logger = ks.getLoggers().newFileLogger( session, "./stock" );
		
		EntryPoint ep = session.getEntryPoint("stock feed");
		
		//insert the code to set the globals as per Rule file for buyTolerance and SelTolerance
...
//				session.fireAllRules();
		...
		try {
			 // insert code to start the StockFeeder
			feeder.start();
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
